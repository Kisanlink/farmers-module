...
type CreateFPORequest struct {
	...
	BusinessConfig map[string]string `json:"business_config" example:"max_farmers:1000,procurement_enabled:true"`
	Metadata       map[string]string `json:"metadata,omitempty" example:"district:Indore,state:MP,established:2024"`
}
...
type RegisterFPORefRequest struct {
	...
	BusinessConfig map[string]string `json:"business_config" example:"credit_limit:500000,payment_terms:net30"`
	Metadata       map[string]string `json:"metadata,omitempty" example:"region:central_india,crop_focus:wheat_soybean"`
}
